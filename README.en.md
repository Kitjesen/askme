...
Vision is currently an on-demand camera/VLM branch, not an always-on unified multimodal model. A visual question can capture the current frame and return a short answer, and explicit `auto_capture` can add a scene description to an ordinary prompt. The primary `config.yaml` disables both local vision and cloud VLM by default. The board profile enables local camera/YOLO perception but keeps cloud `vlm_enabled=false` until a validated LiteLLM `vision-scene` alias and dedicated scoped key are provisioned. Raw image/snapshot IDs and capture timestamps are not yet linked back to the committed Turn/Generation, so continuous visual memory and product-grade auditable multimodality are not claimed.
...
LiteLLM is the default LLM control plane. Before starting a native runtime, bootstrap the sidecar, wait for readiness, and provision `LITELLM_BASE_URL` plus a scoped `LITELLM_VIRTUAL_KEY` as documented in [the gateway runbook](docs/LITELLM_GATEWAY.md). Missing proxy credentials fail closed; the product does not silently reconnect to a direct provider.
...
Warm provider-session lifecycle and troubleshooting are documented in [docs/WARM_SESSIONS.md](docs/WARM_SESSIONS.md).
...
- [docs/WARM_SESSIONS.md](docs/WARM_SESSIONS.md)
