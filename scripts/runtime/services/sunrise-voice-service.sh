...
#
# 用法（在 sunrise 机器上）:
#   screen -S askme ./scripts/runtime/services/sunrise-voice-service.sh   # 后台 screen
#   ./scripts/runtime/services/sunrise-voice-service.sh --text            # 文字模式调试
#   ./scripts/runtime/services/sunrise-voice-service.sh --voice           # 语音模式（默认）
#   ./scripts/runtime/services/sunrise-voice-service.sh --voice --robot   # 语音 + 机器人控制
#
# 生产 systemd 入口使用 deploy/askme.service + askme.blueprints.edge_robot。
# 本脚本仅用于 legacy/manual debug。

set -e

ASKME_DIR="${ASKME_DIR:-$(cd "$(dirname "$0")/../../.." && pwd)}"
VENV="$ASKME_DIR/.venv"
