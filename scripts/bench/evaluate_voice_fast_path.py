"""Benchmark evaluator for the deterministic low-latency voice path.
    ...
    if transport == "usb_direct":
        components = {
            "usb_direct_speech_leadin_ms": max(
                0.0,
                _as_float(tts_config.get("usb_direct_speech_leadin_seconds")) * 1000.0,
            )
        }
        if bool(tts_config.get("usb_direct_persistent_stream", False)):
            components["usb_direct_stream_start_grace_ms"] = max(
                0.0,
                _as_float(tts_config.get("usb_direct_stream_start_grace_seconds"))
                * 1000.0,
            )
...
def _build_projection(
    *,
    ...
    route_p50 = _as_float(route_stats.get("p50"))
    route_p95 = _as_float(route_stats.get("p95"))
    cache_p50 = _as_float(cache_stats.get("p50"))
    cache_p95 = _as_float(cache_stats.get("p95"))
    common.update(
        {
            "p50": round(
...
def _summarize_physical_evidence(
    ...
    *,
    ...
    if len(latencies) < MIN_PHYSICAL_TRIALS:
        return summary
    if _as_float(summary["p95"]) > FAST_PATH_PHYSICAL_P95_MS:
...
def _acceptance_failures(
    *,
    ...
    if _as_int(benchmark.get("false_fast_action_routes"), default=-1) != 0:
