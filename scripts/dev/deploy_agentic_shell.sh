...
# Usage: bash scripts/dev/deploy_agentic_shell.sh [--no-restart] [--no-tests]
#
# Environment overrides:
#   REMOTE=user@host   (default: sunrise@192.168.66.190)
#   RPATH=remote/path  (default: /home/sunrise/data/inovxio/askme)

set -euo pipefail

REMOTE="${REMOTE:-sunrise@192.168.66.190}"
RPATH="${RPATH:-/home/sunrise/data/inovxio/askme}"
RESTART=1
RUN_TESTS=1
...
if [ "${RESTART}" = "1" ]; then
    ...
    ssh ${REMOTE} "
        if systemctl is-active --quiet askme 2>/dev/null; then
            ...
            echo 'No active service — start with: screen -S askme ./scripts/runtime/services/sunrise-voice-service.sh'
        fi
    "
else
