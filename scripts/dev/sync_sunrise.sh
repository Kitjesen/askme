...
# Usage: bash scripts/dev/sync_sunrise.sh
#
...
#   REMOTE_DIR=/remote/askme     (default: /home/sunrise/data/inovxio/askme)

set -euo pipefail

REMOTE="${REMOTE:-sunrise@192.168.66.190}"
REMOTE_DIR="${REMOTE_DIR:-/home/sunrise/data/inovxio/askme}"
...
for f in pyproject.toml requirements.txt README.md; do
    if [ -f "$LOCAL_DIR/$f" ]; then
        scp -q "$LOCAL_DIR/$f" "$REMOTE:$REMOTE_DIR/$f"
    fi
done
if [ -f "$LOCAL_DIR/prompts/SOUL.md" ]; then
    ssh "$REMOTE" "mkdir -p '$REMOTE_DIR/prompts'"
    scp -q "$LOCAL_DIR/prompts/SOUL.md" "$REMOTE:$REMOTE_DIR/prompts/SOUL.md"
fi
...
echo ""
echo "=== Done ==="
echo "  Restart: ssh $REMOTE 'tmux kill-session -t askme; tmux new-session -d -s askme bash -c \"cd $REMOTE_DIR && source .venv/bin/activate && set -a; [ -f .env ] && source .env; set +a; python -m askme --legacy\"'"
