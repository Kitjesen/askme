"""CyclonePulse — 纯 CycloneDDS 数据总线，不依赖 ROS2.

与 Pulse (rclpy) 相同的 PubSubBase 接口，但底层直接用 cyclonedds Python 包。
只需要 libddsc.so + CYCLONEDDS_HOME，不需要 source /opt/ros/humble/setup.bash。

Usage::

    bus = CyclonePulse(cfg)
...
class CyclonePulse(PubSubBase):
    """纯 CycloneDDS 数据总线 — 不依赖 ROS2.
    ...
    async def start(self) -> None:
        ...
        if not self.available:
            logger.info("CyclonePulse: disabled (cyclonedds not available)")
            return
        ...
        self._poll_thread = threading.Thread(
            target=self._poll_loop, name="cyclone_pulse_poll", daemon=True,
        )
        self._poll_thread.start()
        self._started = True
        logger.info("CyclonePulse: started (node=%s, topics=%d, NO ROS2)",
    ...
    async def stop(self) -> None:
        ...
        self._dp = None
        self._started = False
        logger.info("CyclonePulse: stopped (total_msgs=%d)", self._msg_count)
    ...
    def health(self) -> dict[str, Any]:
        h = {
            ...
            "backend": "cyclonedds",
            "topics": self._build_topics_health(),
        }
        return h
