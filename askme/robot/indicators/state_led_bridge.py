...
from askme.robot.indicators.led_controller import LedController, LedStateKind, NullLedController
...
_AGENT_STATE_TO_LED: dict[str, LedStateKind] = {
    "idle":       LedStateKind.IDLE,
    "listening":  LedStateKind.LISTENING,
    "processing": LedStateKind.PROCESSING,
    "speaking":   LedStateKind.SPEAKING,
    "muted":      LedStateKind.MUTED,
}
...
class StateLedBridge:
    ...
    safety:
        Optional SafetyPort — provides ESTOP state.
    led:
    ...
    def resolve(self) -> LedStateKind:
        ...
        if _agent_state_value(agent_state) == "muted":
        ...
        return _AGENT_STATE_TO_LED.get(_agent_state_value(agent_state), LedStateKind.IDLE)
...
def _agent_state_value(state: object) -> str:
