...
def intent_route_payload(
    ...
    *,
    ...
    ``include_content=False`` is the production voice-trace boundary: route
    structure remains observable while transcripts, matched phrases, reply
    previews, and provider evidence stay out of the trace payload.
    """
    ...
    if include_content:
        payload["raw_text_preview"] = _preview(intent.raw_text)
    if intent.skill_name:
        payload["skill_name"] = intent.skill_name
    if include_content and intent.command:
        payload["command"] = intent.command
    if include_content and intent.trigger_phrase:
    ...
    if include_content and intent.route_evidence:
        payload["route_evidence"] = intent.route_evidence
    if include_content and intent.reply_text:
