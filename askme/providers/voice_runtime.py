"""Provider bridge from askme front-door turns into askme-edge-service."""
...
from askme.voice.core.generated_contracts import AskmeEdgeVoiceContract
    ...
    def __init__(self, config: dict[str, Any] | None = None) -> None:
        ...
        if (self.enabled or self.text_enabled) and not self._base_url:
            logger.warning(
                ...
                "(CORTEX_VOICE_URL not set in .env?); bridge is disabled. "
                "Set runtime.voice_bridge.base_url or CORTEX_VOICE_URL to activate."
            )
            self.enabled = False
            self.text_enabled = False

        # Fast connectivity pre-check: if the upstream is unreachable at init
        # time, keep the configured enablement and let the normal circuit
        # breaker/recovery path decide later turns.
        if (self.enabled or self.text_enabled) and self._base_url:
            try:
                ...
                logger.warning(
                    ...
                    "will retry on later turns. Ensure cortex-voice is running on port 5100.",
                    self._base_url,
                    _startup_exc,
                )
    ...
    def handle_voice_text(
        ...
        *,
        ...
        return self.handle_turn(
            text,
            enabled=self.enabled,
            session_id=conversation_session_id or session_id or self._session_id,
    ...
    def handle_text_input(
        ...
        *,
        ...
        return self.handle_turn(
            ...
            session_id=conversation_session_id or session_id or self._text_session_id,
