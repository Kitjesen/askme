...
def _clean_int(value: Any, fallback: int, *, minimum: int = 20, maximum: int = 300) -> int:
    try:
        parsed = int(value)
...
class AssistantPersona:
    ...
    def from_config(cls, raw: dict[str, Any] | None) -> AssistantPersona:
        ...
        return cls(
            robot_name=_clean_text(data.get("robot_name"), cls.robot_name),
            product_name=_clean_text(data.get("product_name"), cls.product_name),
            customer_name=_clean_text(data.get("customer_name"), ""),
            operator_audience=_clean_text(data.get("operator_audience"), cls.operator_audience),
            role=_clean_text(data.get("role"), cls.role),
            ownership_label=_clean_text(data.get("ownership_label"), ""),
            speaking_style=_clean_text(data.get("speaking_style"), cls.speaking_style),
            max_reply_chars=_clean_int(data.get("max_reply_chars"), cls.max_reply_chars),
        )
