...
_FIELD_ACTION_AUDIT_GENESIS = "GENESIS"
_FIELD_ACTION_AUDIT_HASH_ALG = "sha256"
...
_FIELD_ACTION_AUDIT_SIGNATURE_ALG = "hmac-sha256"
_FIELD_DEVICE_SIGNATURE_ALG = "hmac-sha256"
_FIELD_DEVICE_SIGNATURE_FIELDS = {
    "device_signature",
    "signature",
    "x_signature",
...
class FieldOperationsService:
    ...
    def __init__(
        ...
        *,
        ...
        self._robot_id = str(cfg.get("robot_id") or os.getenv("ASKME_ROBOT_ID") or "thunder-1")
        self._robot_name = str(cfg.get("robot_name") or os.getenv("ASKME_ROBOT_NAME") or "Thunder")
    ...
    def _append_action_audit_record(self, event: dict[str, Any], record: dict[str, Any]) -> None:
        ...
        try:
            ...
            sequence, previous_hash = _next_field_action_audit_checkpoint(self._action_audit_path)
            ...
            if not self._action_audit_swallow_errors:
                raise
...
def sign_field_device_payload(body: dict[str, Any], *, secret: str) -> str:
    ...
    encoded = json.dumps(
        _unsigned_field_device_payload(body),
        ensure_ascii=False,
        sort_keys=True,
        separators=(",", ":"),
    ).encode("utf-8")
...
def _unsigned_field_device_payload(body: dict[str, Any]) -> dict[str, Any]:
    return {
        key: value
        for key, value in body.items()
        if key not in _FIELD_DEVICE_SIGNATURE_FIELDS
    }


def _field_device_signature_value(body: dict[str, Any]) -> str:
...
def _field_device_signature_timestamp(body: dict[str, Any]) -> float | None:
    return _parse_timestamp(body.get("device_signature_timestamp") or body.get("signature_timestamp"))


def _field_device_id(body: dict[str, Any], normalized: dict[str, Any]) -> str:
...
def _field_action_audit_hash(record: dict[str, Any]) -> str:
...
def _field_action_audit_signature(record: dict[str, Any], *, secret: str) -> str:
    payload = dict(record)
    payload.pop("record_signature", None)
    encoded = json.dumps(
        payload,
...
def _next_field_action_audit_checkpoint(path: Path) -> tuple[int, str]:
    if not path.exists():
        return 1, _FIELD_ACTION_AUDIT_GENESIS
    latest_hash = _FIELD_ACTION_AUDIT_GENESIS
    sequence = 0
...
def _field_action_audit_counts_by_event(events: list[dict[str, Any]]) -> dict[str, int]:
    counts: dict[str, int] = {}
    for event in events:
        event_id = str(event.get("event_id") or "")
