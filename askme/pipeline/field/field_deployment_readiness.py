...
def build_field_deployment_readiness(
    *,
    ...
    device_onboarding: dict[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    device_onboarding_summary = _device_onboarding_summary(device_onboarding)
    ...
    gates = {
        ...
        "device_onboarding_report_available": device_onboarding_summary["available"],
        "device_onboarding_no_blockers": device_onboarding_summary["blocked"] == 0,
        "device_onboarding_has_ready_device": device_onboarding_summary["ready"] > 0,
        "device_onboarding_all_ready": device_onboarding_summary["all_ready"],
    ...
    if device_onboarding_summary["available"]:
        if device_onboarding_summary["blocked"] > 0:
            warnings.append("field device onboarding has blocked devices")
        if device_onboarding_summary["manual_check"] > 0:
            warnings.append("field device onboarding still needs manual review")
        if gates["device_registry_configured"] and device_onboarding_summary["ready"] == 0:
            warnings.append("field device onboarding has no ready devices")
    elif gates["device_registry_configured"]:
        warnings.append("field device onboarding report is not available")
    ...
    return {
        ...
        "device_onboarding": device_onboarding_summary,
...
def _add_production_blockers(blockers: list[str], gates: dict[str, bool]) -> None:
    required_gates = {
        ...
        "device_onboarding_report_available": "production requires device onboarding readiness evidence",
        "device_onboarding_no_blockers": "production requires device onboarding without blocked devices",
        "device_onboarding_has_ready_device": "production requires at least one ready onboarded field device",
        "device_onboarding_all_ready": (
            "production requires every registered field device to be onboarded, "
            "observed, trusted, and bound to a managed object"
        ),
...
def _device_onboarding_summary(payload: dict[str, Any] | None) -> dict[str, Any]:
    if not isinstance(payload, dict):
        return {
            "available": False,
            "report_type": "",
    ...
    devices = [dict(item) for item in raw_devices if isinstance(item, dict)]
    registered = _int_or_zero(raw_summary.get("registered"))
    observed = _int_or_zero(raw_summary.get("observed"))
    ready = _int_or_zero(raw_summary.get("ready"))
    blocked = _int_or_zero(raw_summary.get("blocked"))
    manual_check = _int_or_zero(raw_summary.get("manual_check"))
    total_device_count = len(devices) if devices else max(registered, observed, ready + blocked + manual_check)
    return {
        "available": True,
        ...
        "all_ready": bool(total_device_count > 0 and ready == total_device_count and blocked == 0 and manual_check == 0),
        "next_actions": [
            str(item)
...
def _next_actions(blockers: list[str], warnings: list[str]) -> list[str]:
    ...
    if any("device onboarding" in item for item in blockers + warnings):
        actions.append("Open device onboarding readiness and bind each device to a managed object")
