...
4. **如果没找到，快速 360° 扫描**：
   `scan_around(question="有没有[目标物体]")` — 边旋转边拍 4 个方向，一次性分析（~10s）
5. 四周都没找到时，**推理可能位置**：
...
**重要规则（必须严格遵守）：**
- **最多调用 3 次工具**：1次 scan_around + 1次 look_around + 1次 move_robot(go_to)。然后必须给结论
- **禁止反复换问法重试**——同样的区域看过就是看过，不要换个问题再看一遍
- 看一次没找到 → 直接说"当前位置没看到[物体]"，建议用户指定位置
- 优先 `scan_around`（一次看全），不要多次 `look_around`
- 找到后描述方位（左/右/前方/桌上）
