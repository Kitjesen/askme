...
2. 用 `scan_around(question="有没有安全隐患？检查：地面积水/漏水、障碍物挡路、烟雾/火花、电线裸露、设备异常")` 做快速 360° 扫描（~10s 一次性完成，比逐个方向看快 3-4 倍）
3. 如需深入查看某个方向，用 `move_robot(action="rotate")` + `look_around(question=...)` 补充细节
