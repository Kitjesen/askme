...
5. **情景记忆日志** — `bash` 执行 `cat data/episodic_journal.jsonl | tail -20` 查看最近事件

**查询流程：**

1. 理解用户问什么（设备？事件？流程？地点？时间？）
2. 用 `read_file` 直接读对应的 markdown/json 文件
3. 如果不确定在哪个文件，先读 `data/qp_memory/knowledge/incidents.md`（最常查）
4. 从文件内容中找到相关信息，用简短语言回答
5. 如果没找到，诚实说"没有相关记录"
