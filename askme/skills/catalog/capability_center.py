...
_GROUPS: dict[str, tuple[str, str]] = {
    ...
    "incident": ("异常处置", "摔倒、卡住、故障、挡路等机器人自身异常闭环。"),
    "security": ("安防巡查", "夜间陌生人、违停、人群聚集、烟火异常和取证通知。"),
...
_SPECS: dict[str, CapabilitySpec] = {
    "patrol_scan": _spec("patrol_scan", "巡检指定区域", "patrol", "到指定区域采集巡检信息。", "P0"),
    "patrol_report": _spec("patrol_report", "生成巡检报告", "patrol", "汇总本轮巡检结果、异常和证据。", "P0"),
    "navigate": _spec("navigate", "前往指定位置", "patrol", "把目标地点转换为导航任务。", "P0", requires_approval=True),
    "nav_cancel": _spec("nav_cancel", "取消导航", "patrol", "取消当前导航或带路任务。", "P0"),
    "nav_query": _spec("nav_query", "查询导航状态", "patrol", "查询当前位置、目标和导航进度。", "P0"),
    "robot_home": _spec("robot_home", "返回待命点", "patrol", "返回待命点或充电区域。", "P0", requires_approval=True),
    "robot_estop": _spec("robot_estop", "紧急停止", "incident", "立即进入急停安全状态。", "P0"),
    "dog_control": _spec("dog_control", "机器狗姿态控制", "incident", "站立、坐下、趴下等高风险动作。", "P0", requires_approval=True),
    "report_fall_unrecoverable": _spec("report_fall_unrecoverable", "摔倒无法恢复", "incident", "播报、拍照、通知保安并归档。", "P0"),
    "report_stuck": _spec("report_stuck", "卡住无法运动", "incident", "识别卡住并通知人工处理。", "P0"),
    "report_malicious_blocking": _spec("report_malicious_blocking", "人为恶意挡路", "incident", "暂停机器人、保持安全距离、通知保安并归档证据。", "P0"),
    "report_motor_fault": _spec("report_motor_fault", "关节电机故障", "incident", "停止任务、记录故障码并通知维护。", "P0"),
    "environment_report": _spec("environment_report", "现场环境播报", "security", "播报当前环境、异常和传感器摘要。", "P1"),
    "find_person": _spec("find_person", "寻找人员", "security", "按描述寻找人员并回报位置。", "P1"),
    "find_object": _spec("find_object", "寻找物品", "security", "按描述寻找现场物品或目标。", "P1"),
    "safety_check": _spec("safety_check", "安全检查", "security", "执行任务前的基础安全检查。", "P0"),
    "detect_night_intruder": _spec("detect_night_intruder", "夜间陌生人检测", "security", "夜间识别窗户、角落等敏感区域人员。", "P0", requires_approval=True),
    "detect_illegal_parking": _spec("detect_illegal_parking", "车辆违停检测", "security", "识别主通道和非停车区违停车辆。", "P0", requires_approval=True),
    "detect_fire_smoke": _spec("detect_fire_smoke", "火灾烟雾监测", "security", "接入烟感、温度和视觉烟火识别。", "P0", requires_approval=True),
    "detect_crowd_gathering": _spec("detect_crowd_gathering", "人群聚集检测", "security", "识别人数、停留时长和复巡后是否仍聚集。", "P1"),
    "inspect_trash_bin": _spec("inspect_trash_bin", "垃圾桶满溢检测", "operations", "定点拍照识别垃圾桶是否满溢。", "P0"),
    "check_location": _spec("check_location", "查询当前位置", "space", "查询机器狗当前所在区域或点位。", "P0"),
    "mapping": _spec("mapping", "地图建图", "space", "启动或辅助园区建图流程。", "P1", requires_approval=True),
    "lookup_place": _spec("lookup_place", "查询园区点位", "space", "查询商户、楼宇、卫生间、停车区。", "P0"),
    "recommend_route": _spec("recommend_route", "推荐路线", "space", "按当前位置和目的地生成路线。", "P0"),
    "greet_person": _spec("greet_person", "访客问候", "visitor", "在服务点向访客发起问候。", "P0"),
    "offer_wayfinding_help": _spec("offer_wayfinding_help", "主动问路服务", "visitor", "固定问询点识别访客停留后主动询问。", "P0"),
    "answer_wayfinding": _spec("answer_wayfinding", "语音指路", "visitor", "查询园区语义地图并播报路线。", "P0"),
    "escort_visitor": _spec("escort_visitor", "机器狗带路", "visitor", "低速引导访客前往确认地点。", "P1", requires_approval=True),
    "follow_person": _spec("follow_person", "跟随人员", "visitor", "低速跟随指定人员。", "P1", requires_approval=True),
    "repeat_last": _spec("repeat_last", "重复上一句", "voice", "访客没听清时重复上一条播报。", "P0"),
    "mute_mic": _spec("mute_mic", "关闭麦克风", "voice", "临时关闭语音输入。", "P0"),
    "unmute_mic": _spec("unmute_mic", "打开麦克风", "voice", "恢复语音输入。", "P0"),
    "stop_speaking": _spec("stop_speaking", "停止播报", "voice", "立即停止当前语音播报。", "P0"),
    "volume_up": _spec("volume_up", "调大音量", "voice", "提高播报音量。", "P1"),
    "volume_down": _spec("volume_down", "调小音量", "voice", "降低播报音量。", "P1"),
    "volume_reset": _spec("volume_reset", "恢复默认音量", "voice", "恢复默认播报音量。", "P1"),
    "speed_up": _spec("speed_up", "加快语速", "voice", "提高播报语速。", "P1"),
    "speed_down": _spec("speed_down", "降低语速", "voice", "降低播报语速。", "P1"),
...
_PLANNED: list[CapabilitySpec] = [
    _spec("start_patrol_route", "开始巡检路线", "patrol", "启动客户配置的固定巡检路线。", "P0", requires_approval=True),
    _spec("inspect_point", "检查指定点位", "patrol", "检查设备、门口、垃圾桶或停车区。", "P0"),
    _spec("notify_security_group", "通知安保群", "security", "发送钉钉、企微或短信给安保。", "P0"),
    _spec("notify_cleaning_group", "通知保洁群", "operations", "通知保洁处理卫生问题。", "P0"),
    _spec("enable_capability", "启用能力", "governance", "管理员启用能力包。", "P1", requires_approval=True),
    _spec("view_skill_audit_log", "查看调用记录", "governance", "查询能力调用、结果和操作员。", "P1"),
]
...
def _build_scenario_blueprints(
    *,
    ...
    try:
        from askme.pipeline.field.field_scenarios import FIELD_SCENARIOS
    except Exception:
        return {
            "summary": {
                "scenario_count": 0,
    ...
    return {
        ...
        "items": items,
        "policy": {
            "source": "askme.pipeline.field.field_scenarios",
            "scenario_requires_all_required_skills": True,
            "runtime_entry": "field_event_trigger",
