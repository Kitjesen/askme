...
class TemporalQueryTool(BaseTool):
    ...
    def __init__(self, temporal_memory_client: Any | None = None) -> None:
        self._temporal_memory_client = temporal_memory_client

    def set_temporal_memory_client(self, temporal_memory_client: Any) -> None:
        self._temporal_memory_client = temporal_memory_client
    ...
    def execute(
        ...
        **kwargs: Any,
        ...
        if not nav_url and self._temporal_memory_client is None:
        ...
        if self._temporal_memory_client is not None:
            data = self._temporal_memory_client.query_temporal_observations(params)
            if "error" in data:
                return f"[时空记忆] 查询失败: {data['error']}"
            return self._format_observations(data, label=label, since=since)
...
def register_temporal_tools(
    registry: Any,
    temporal_memory_client: Any | None = None,
) -> None:
    """Register temporal memory tools with the tool registry."""
    registry.register(TemporalQueryTool(temporal_memory_client=temporal_memory_client))
