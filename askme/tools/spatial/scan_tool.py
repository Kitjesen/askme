...
class ScanAroundTool(BaseTool):
    ...
    def _request_rotation_scan(self) -> str:
        ...
        params = {"speed_deg_per_sec": 45}
        if self._robot_control_client is not None:
            result = self._robot_control_client.dispatch_capability("scan_360", params)
        else:
            from askme.tools.robot.runtime_api import call_runtime_api

            # Standalone fallback. Runtime-built tools receive RobotControlPort instead.
            result = call_runtime_api("control", "POST", "/api/v1/control/executions", {
...
def register_scan_tools(
    ...
    """Register scan tools."""
    tool = ScanAroundTool(robot_control_client=robot_control_client)
    if vision:
